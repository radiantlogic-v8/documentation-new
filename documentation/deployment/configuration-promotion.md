...
Radiant Logic's configuration promotion feature allows you to seamlessly transfer configuration changes across different Identity Data Management instances. This enables you to synchronize updates from one environment to another, such as promoting changes from a development environment to QA or production environments. You can select exactly which staged resources to promote and validate dependencies before exporting. 

This feature is supported in Identity Data Management version 8.4.0 and higher.
...
- For SaaS Identity Data Management environments, customers must [configure their promotion pipeline](/../../../eoc/latest/promotion-pipelines/configuration-promotion) in the Environment Operations Center (EOC). Note that this feature is supported in EOC version 1.4.0 or higher. 
