...
`$ <RLI_HOME>/bin/advanced/cluster.sh system-check`

An example would return:

`PASSED Maximum number of open file descriptors [score: 100%]
                  65536 (minimum 48k)`

If you need to adjust the value, use these steps:

1. As root, open the system's /etc/security/limits.conf file.
2. Add two lines that set the hard and soft limits for the number of file descriptors (nofile) for the Directory Server user. The soft limit sets how many files the user has available by default; the user can manually adjust that setting until they hit the hard limit.
