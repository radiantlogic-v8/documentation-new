...
`-timeout <timeout>`
<br> This optional property specifies the maximum duration (in minutes) that vdsconfig will wait for the empty-queues operation to complete before failing with a timeout error. If omitted, the command uses its default timeout, which may not be sufficient for large queues. 
...
Agents are also used for CPLDS sync processes. CPLDS Agents run on follower-only cluster nodes and multiple agents can be running at the same time. The more agents the better in order to distribute the load. For more details, see the [RadiantOne CPLDS Guide](/cplds-guide/introduction).
...
>[!note] For Windows command line, some property values require double quotes surrounding them. In this case, use `\"` to escape inner quotes. When setting multi-valued properties such as `addEventExcludedAttributes`, `modifyEventExcludedAttributes`, or `deleteEventExcludedAttributes` (available for real-time persistent cache using an LDAP connector), wrap the entire list in double quotes and escape each attribute name's quotes with `\"`. The correct format for `-propertyvalue` is `"[\"member\",\"memberof\",\"whenCreated\"]"` — **not** `["member","memberOf","whenCreated"]`.
