---
title: CFS
description: Configuring SSO for ServiceNow with CFS
---
...
*   Look for the application **ServiceNow**
*   Click **Configure**.

![](media/servicenow-configure.png)
...
![](media/servicenow-tab-general.png)

### Parameters

The **Parameters** tab contains the list of settings required to configure the application.

*   **Issuer** - Name used to identify CFS on ServiceNow.
*   **Company** - Enter the sub domain created on ServiceNow. Usually your company name.

![](media/servicenow-tab-parameters.png)
...
![](media/servicenow-tab-mappings.png)
...
![](media/servicenow-tab-filter.png)
...
![](media/servicenow-tab-certificate.png)

>[!note] After the _Jakarta_ release, ServiceNow has started enforcing admins to test the application before activating your IdP. Since we do not set the `SessionIndex` parameter in the SAML token, the test will always fail and it will not allow you to activate your IdP.  
To work around this issue:

*   Go to _System Properties_
*   Add the parameter `glide.authenticate.multisso.test.connection.mandatory`
