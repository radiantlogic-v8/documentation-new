...
Templates are used to define data sources and each data source type requires a template. RadiantOne includes default templates which can be used when creating data sources from Control Panel > Setup > Data Catalog > Data Sources.

![New Data Source](Media/new-data-source.jpg)

For database data sources, a JDBC driver is associated with each template. RadiantOne includes the following JDBC drivers associated with their respective template: Oracle (thin), Oracle oci, Microsoft SQL Server (driver from Microsoft and JTDS), HSQL, MySQL (supports MariaDB too), PostGres, Sybase, and Derby.
