...
-	[VSTS45636]: Added number of CPU cores per node to the control panel dashboard.
-	[VSTS45783]: Updated MGraph client to use v1.0 API instead of beta.
-	[VSTS45951]: Added move node up and move node down buttons to the View Designer.
-	[VSTS46232]: Added multiple expressions to computed attributes. 
-	[VSTS46233]: 
-	[VSTS46373]: Added an unsaved changes warning message that will appear when navigating away from the SCIM tab. 
-	[VSTS46486]: Custom limits time limit is now re-enabled.
...
-	[VSTS46740]: Added support of the latest secure hash algorithm SHA-3 for password protection when FID is running in FIPS mode.
...
-	[VSTS46918]: Added a utility tool to decrypt the FID exported LDIFz file and the decrypted file can then work with any third-party LDIF reader.
-	[VSTS46926]: Added support for snowflake as a Database for realtime refresh.
-	[VSTS46929]: Added a utility tool to decrypt the FID exported LDIFz file, which is zipped in encryption form.
-	[VSTS46932]: Added FID_SERVER_JOPTS to runVDSServer.sh and CP_SERVER_JOPTS to runWebAppServer.sh
-	[VSTS46938]: Added support for MFA enabled users to create service accounts using Azure AD Initialization tool and provided an option to enter data source name for the custom data source created in FID.
-	[VSTS46941]: Improved sensitive information masking in logs.
-	[VSTS46952]: Improved to not allow the root account CN=Directory Manager to be deleted. 
-	[VSTS46957]: Removed DSML/SPML Service from the new naming context popup.  Removed SPML targets section from Settings -> Server Front End -> Other Protocols.
-	[VSTS46968]:
-	[VSTS46990]: Added a confirmation dialog that will pop up message when deactivating naming contexts and/or caches.
-	[VSTS47057]: Added a license expiration message that now appears in the control panel.
-	[VSTS47083]: Updated Cyberark documentation on Support Portal.
...
-	[VSTS46558]: Fixed an issue that caused a global sync toplogy to not be migrated if its source or destination points to entry that is not a naming context but is instead a child within a naming context.
-	[VSTS46819]: Updated profile-password for mgraph Azure AD users to reflect that it is required for creation.
-	[VSTS46826]: Fixed an issue that caused an UnsupportedOperationException during ZK export on Windows.
...
-	[VSTS46942]: Fixed an issue where GIB stats would timeout while upload was ongoing.	[VSTS46956]: 
-	[VSTS46965]: Fixed an issue so that after enabling vds access logs, logging will now start immediately. There is no need to restart FID.
-	[VSTS46967]: Fixed an issue where the connection to failover nodes within a FID cluster was not functioning in resync-utils.bat tool, which results in inconsistent state when the leadership of node changes.
...
-	[VSTS46979]: Fixed the issue that prevented copying of sync topology from one environment to another. Vdsconfig resource traverse is now considering parent resources as dependencies (along with child resources).
-	[VSTS46980]: 
-	[VSTS46982]: 
