...
- [IV4-22]: Improvement to allow attributes to be treated as case-sensitive during the cache validation/update process, meaning that changing the case of attribute values will be reflected with the same case in the cached entry. 
...
- [IV4-47]: Bundled JDBC drivers will now be updated when applying patches.
- [IV4-52]: License expiration date is now shown for subscription licenses in both the CLI tool and the Server Control Panel.
...
- [IV4-69]: SCIMv2 API to the RadiantOne service now accepts either LDAP or SCIM/ISO timestamp formats in query filters.
