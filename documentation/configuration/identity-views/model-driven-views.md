...
To configure Link Parameters, Base Parameters, Additional Clause and configure calls for Stored Procedures, click **EDIT** in the Configuration Parameters section.

- Link Parameters: are only relevant when the view definition has more than one level because this is the parameter that is used to link the entry to the parent. If there is only one level in the tree, this parameter is irrelevant. If RadiantOne has generated UPPER, you can remove it from here if needed. The UPPER is generated here for case sensitive databases. If your database is not case sensitive, then the UPPER can be removed (along with the corresponding set of parentheses). If your database is case sensitive, then you must make sure that the attribute that is indexed (and is used in the link parameter) uses UPPER case. Otherwise, performance of the virtual view is unpredictable.<br>

- Base Parameters: This parameter allows you to remove UPPER (that is generated by default) for base searches and updates. The UPPER is generated here for case sensitive databases. If your database is not case sensitive, then the UPPER can be removed. If your database is case sensitive, then you must make sure that the attribute that is indexed (and is used in the base parameter) uses the UPPER case. Otherwise, performance of the virtual view (during a base search) is unpredictable. Since some databases are case sensitive, RadiantOne transforms primary key attribute values to upper case before comparing. This is only applicable when the database key is a character data type. “UPPER” is generated automatically for the following databases: Oracle, SQL MX, MS SQL Server, Sybase, Interbase, and DB2. If you do not want UPPER to be used for searches and updates (because your database is not case sensitive), you can use the Base parameters to remove it. Below is an example of the parameter that is generated for a database (for the search). If the database is case sensitive and the CN attribute is indexed (and the index stores the value in upper case), then the UPPER generated on the left side of the “=” can be removed and performance is improved (also remove the leading and ending parentheses). <br> (UPPER(SCOTT.PEOPLE.CN)=UPPER(@@[cn : VARCHAR2(60)])<br>If you are not able to remove the UPPER then you should consider using cache at the level of RadiantOne to improve performance. RadiantOne queries the database the first time (the UPPER is used), the result is cached and future queries receive the information from cache.<br>

- Additional Clause: The **Additional Clause** setting lets you append backend-specific query criteria for database and LDAP backends. For database backends, RadiantOne sends the clause exactly as entered, so use syntax that is valid for your database. You can use an `ORDER BY` clause, such as `ORDER BY l`, to return entries in a consistent order. Consistent ordering allows RadiantOne to merge entries with the same RDN; otherwise, duplicate RDNs can be returned when the RDN attribute is not unique. You can order by any attribute in the virtual view, but use the physical database column name rather than the remapped virtual attribute name. For LDAP backends, enter the LDAP filter prefixed with `filter:`, for example: `filter:(&(employeenumber=11)(givenname=henry))`.<br>
...
For model-driven identity views that use a Microsoft Entra ID (`mgraph`) backend, you can configure Microsoft Graph query options in the **ADVANCED SETTINGS** tab > **Other Settings** section.

Use a Graph API Filter to limit the entries returned by an identity view. The filter is sent directly to Microsoft Graph list-object requests, such as requests for users or groups, through the `$filter` query parameter. Because the filter bypasses the standard LDAP-to-Microsoft Graph conversion layer, you can use native Microsoft Graph filter syntax. This also helps address conversion-layer limitations, such as unsupported LDAP `NOT` filters.

The following settings are available:

- Graph API Filter: Enter a valid Microsoft Graph `$filter` expression. For example:

  ```
  department eq 'Engineering'
  ```

- Enable Microsoft Graph Advanced Queries: Select this option if the filter uses advanced query operators or functions, such as `not` or `endsWith`. Advanced queries require the `ConsistencyLevel: eventual` and `$count=true` request headers. For example, the following filter requires advanced queries:

  ```
  not(startswith(displayName,'group4'))
  ```

To configure a Graph API Filter:

1. Go to Control Panel > Setup > Directory Namespace > Namespace Design.
2. In the namespace tree, select the content or container node created from the Microsoft Entra ID backend schema.
...
5. If the filter uses advanced query capabilities, select Enable Microsoft Graph Advanced Queries.
6. Click SAVE.
