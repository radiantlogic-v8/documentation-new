...
## When and why do you need a cache with RadiantOne?
...
Here is a properly formatted Markdown version with consistent spacing and cleaner structure. Nested bullets are indented so they render as part of the parent item, and the table is separated by a blank line for reliable Markdown parsing. [github.github](https://github.github.com/gfm/)

- **Enable Password Policy Enforcement** - If you store user passwords in cache and use the cache for authentication, you can have RadiantOne enforce password policies instead of delegating password validation to the backend directory. Enable this option, then define the password policy to enforce. Enabling this option also makes the **Cross-Store Password Policy** option available. For details on password policies, see the [RadiantOne System Administration Guide](/sys-admin-guide/01-introduction).

- When you enable password policy enforcement on a persistent cache, the `userPassword` attribute is automatically added to the **Extension Attribute** property, and you can optionally enable **Password Write Back**. If **Password Write Back** is enabled and a password modify request is sent to RadiantOne, RadiantOne attempts to update the password in the backend. In some cases, having two levels of password policies can create inconsistencies between the cache image and the underlying backend. These cases are described in the following table.

- **Cross-Store Password Policy** - Enabling this policy ensures that password rules are enforced consistently across both the cache and the underlying backend, eliminating the dual-policy inconsistencies described in the table below. This option is available only when **Enable Password Policy Enforcement** is enabled and the backend data source is an HDAP store configured with the same password policy. When enabled:
  - **Password Write Back** is automatically turned on and cannot be disabled.
  - The `userPassword` attribute is automatically removed from **Extension Attributes**, because password attributes are managed directly in the backend instead of being cached as extensions.

| Password Change Event | Password Write Back Enabled? | Cross-Store Password Policy Enabled? | Expected Behavior |
|---|---|---|---|
| Password update via a Modify Request sent to RadiantOne | No | No | The password is updated in the persistent cache if it complies with the persistent cache password policy. Otherwise, the update fails. The password update is not sent to the backend. If the cache update succeeds, the password in the persistent cache differs from the password in the backend. |
| Password update via a Modify Request sent to RadiantOne | Yes | No | The password update is sent to the backend. If the password update fails in the backend, the password in the persistent cache is not updated. If the password update succeeds in the backend, the password is also updated in the persistent cache. |
| Password update via a Modify Request sent to RadiantOne | Yes (automatic) | Yes | The password update is sent to the backend HDAP store and validated against the shared password policy. If the update succeeds in the backend, the persistent cache is updated accordingly. Password rules are enforced consistently across both layers, preventing inconsistencies. |
| Password is updated directly in the backend (outside of RadiantOne) | N/A | No | Through the persistent cache refresh process, the password is updated in the persistent cache. If the account was locked in the persistent cache because of a password policy enforced at the cache layer, it is unlocked by the cache refresh process after a successful password update in the backend. Password strength defined in the persistent cache password policy is not enforced because the password change originated in the backend. |
| Password is updated directly in the backend (outside of RadiantOne) | N/A | Yes | Through the persistent cache refresh process, the password is updated in the persistent cache. Because both the cache and backend share the same password policy, the update remains consistent across both layers without policy conflicts. |
