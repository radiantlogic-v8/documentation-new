...
### Capturing Metadata from LDAP-Accessible Backends 
...
### Capturing Metadata from JDBC-Accessible Database Backends 
...
### Capturing Metadata from SCIMv2 Backends
...
3.	Choose the column(s) from the source and the matching column in the destination and then click ![An image showing ](Media/Imagebutton.jpg).
...
Objects in a database schema are tables or views and can be removed from the schema file. Objects in an LDAP schema are object classes. Right-click on the desired object and choose Delete. 
